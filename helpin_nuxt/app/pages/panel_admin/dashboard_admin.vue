    ...
    <main class="flex-1 flex flex-col overflow-y-auto relative w-full no-scrollbar">
      <header class="flex justify-between items-center px-6 md:px-10 py-5 border-b border-gray-200 bg-white/90 backdrop-blur-md shadow-sm z-10 sticky top-0">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-xl md:text-2xl font-black text-[#19462D] tracking-tight">Dashboard Admin</h1>
        ...
        <template v-else>
          <section class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div v-for="c in cards" :key="c.label" class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              ...
              <h3 class="text-2xl font-black text-[#19462D]">{{ c.value }}</h3>
            </div>
          </section>
          ...
          <section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-center gap-4">
              ...
              <div class="bg-green-50 p-5 rounded-2xl">
                ...
                <p class="text-3xl font-black text-green-700">{{ rupiah(stats.balance) }}</p>
              </div>
              <div class="grid grid-cols-2 gap-4">
                <div class="bg-blue-50 p-4 rounded-2xl"><p class="text-[10px] font-black text-blue-500 uppercase">Masuk</p><p class="text-lg font-black text-blue-700">{{ rupiah(stats.total_income) }}</p></div>
                <div class="bg-red-50 p-4 rounded-2xl"><p class="text-[10px] font-black text-red-500 uppercase">Keluar</p><p class="text-lg font-black text-red-600">{{ rupiah(stats.total_expense) }}</p></div>
              </div>
            </div>
          </section>
        </template>
...
import { MenuIcon, UsersIcon, BoxIcon, TruckIcon, ReceiptIcon } from 'lucide-vue-next'
...
onMounted(async () => {
  loading.value = true
  try { stats.value = (await fetchStats()) || stats.value } catch (e) { console.error(e) } finally { loading.value = false }
})
