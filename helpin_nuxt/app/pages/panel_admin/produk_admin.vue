...
const load = async () => {
  loading.value = true
  try { products.value = (await fetchProducts()) || [] } catch (e) { console.error(e) } finally { loading.value = false }
}
onMounted(load)

const filtered = computed(() => products.value.filter(p => p.name?.toLowerCase().includes(search.value.toLowerCase())))

// Input bermata uang / angka dengan pemisah ribuan (tampil "250.000", simpan number murni)
...
const imgUrl = (u) => {
  ...
  return u // /media/... served by nuxt same origin
}
